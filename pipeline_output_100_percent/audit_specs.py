...
def main():
    """Main function to run the LLM-assisted audit pass."""
    print("=== Starting LLM-Assisted Audit Pass ===")
    
    # Load the chapter content and extracted specs
    print("Loading chapter and extracted specs...")
    ...
    # Only audit specs that are empty or likely incorrect
    specs_to_audit = []
    for spec in extracted_specs:
        value = spec.get('value', '')
        spec_item = spec['spec_item']
        
        # Check for empty or obviously wrong values
        should_audit = False
        
        if not value or value.strip() == "" or value == "NO_MATCH_FOUND":
            should_audit = True
        elif spec_item == "Port-side exhaust fan module part number" and "NXASFAN-" in value:
            # Missing hyphen in fan module part number
            should_audit = True
        elif spec_item == "Port-side intake fan module part number" and "NXASFAN-" in value:
            # Missing hyphen in fan module part number
            should_audit = True
        elif spec_item == "Regulatory compliance" and "CE Markings" in value:
            # Wrong regulatory compliance value
            should_audit = True
        
        if should_audit:
            specs_to_audit.append(spec)
    
    if not specs_to_audit:
        print("No specs need auditing - all values are present and correct!")
        # Copy the original specs as corrected
        with open("corrected_specs.json", "w") as f:
            json.dump(extracted_specs, f, indent=2)
        return
    
    print(f"Found {len(specs_to_audit)} specs to audit out of {len(extracted_specs)} total")
    ...
    # Merge the corrected values with the original specs
    corrected_specs = []
    corrected_dict = {entry['spec_item']: entry['corrected_value'] for entry in corrected_entries}
    
    for spec in extracted_specs:
        spec_item = spec['spec_item']
        if spec_item in corrected_dict:
            # Use the corrected value
            corrected_specs.append({
                "spec_item": spec_item,
                "value": corrected_dict[spec_item]
            })
        else:
    ...
    print("Saving corrected specs...")
    with open("corrected_specs.json", "w") as f:
        json.dump(corrected_specs, f, indent=2)
    
    print("=== Audit Complete ===")
    print(f"Corrected {len(corrected_entries)} specifications")
    print("Results saved to corrected_specs.json")
